  ...
  return (
    <div>
        ...
        {featuredRooms.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {featuredRooms.map((cat) => (
              ...
              >
                <img
                  src={cat.image || "https://images.unsplash.com/photo-1611892440504-42a792e24d32?auto=format&fit=crop&w=600&q=80"}
                  alt={cat.type}
                  className="w-full h-48 object-cover"
                />
                <div className="p-4">
                  <h3 className="text-lg font-bold text-[#1E3A8A] capitalize">{cat.type} Room</h3>
                  <p className="text-[#D4AF37] font-semibold">From ৳{cat.minPrice} / Night</p>
        ...
        <div className="max-w-6xl mx-auto px-6 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-6">
            {facilities.map((facility) => (
              <div key={facility} className="bg-white shadow rounded-lg px-6 py-4 font-medium text-[#1E3A8A]">
                {facility}
              </div>
            ))}
          </div>
      ...
      <section className="max-w-6xl mx-auto px-6 py-16 text-center">
        ...
        <p className="text-gray-500 italic">"Amazing stay, wonderful staff!" — coming soon with real reviews</p>
      </section>
    </div>
  );
};
